...
生成流程由有限状态机（FSM）管理：
```
INIT → OUTLINE → OUTLINE_REVIEW → WRITING → PRE_REVIEW_INTEGRITY → REVIEWING → CONSISTENCY → [FINAL_INTEGRITY] → DONE
                                     ↑__________________________|（审查不通过回写作，最多3次）
                                     ↑___________________________________________|（一致性不通过回写作，最多2次）
                                                  ↑________________________________|（PRE_REVIEW 不通过回写作修复，最多3次）
```
...
**去重质量度量**：
- `evaluate_dedup_quality(task_id)`：计算所有章节对的向量相似度，> 0.85 判定为重复，输出重复率报告
- 目标：启用记忆层后并行章节内容重复率 < 5%（对比无记忆层基线的 20-30%）

**优雅降级**：
- kuzu（graph provider）不可用 → 退回到内存 dict 存储主题领地（丢失图查询，基础去重仍工作）
- lancedb（vector provider）不可用 → 退回到 numpy 暴力余弦相似度（小规模任务可接受）
