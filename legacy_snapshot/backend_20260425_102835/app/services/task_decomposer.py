...
def _ensure_depth_chapter_count(dag: DAGSchema, *, depth: str) -> DAGSchema:
    ...
    if not bounds or not dag.nodes:
        return dag

    min_chapters, _max_chapters = bounds
    raw_nodes = [node.model_dump() for node in dag.nodes]
    existing_ids = {str(node["id"]) for node in raw_nodes}
    ...
    logger.bind(depth=depth).info(
        "Applied depth chapter guard: primary writers {} -> minimum {}",
        len(primary_writers),
        min_chapters,
    )
    return DAGSchema.model_validate({"nodes": raw_nodes})
...
async def decompose_task(
    *,
    ...
    try:
        ...
        dag = _ensure_depth_chapter_count(dag, depth=depth)
