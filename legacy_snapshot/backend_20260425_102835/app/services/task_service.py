...
async def get_task_detail(
    ...
    *,
    ...
    for n in nodes:
        read = TaskNodeRead.model_validate(n)
        node_meta = routing_node_meta.get(str(n.id), {})
        result_meta = routing_results.get(str(n.id), {})
        read.required_capabilities = list(node_meta.get("required_capabilities", []))
        read.preferred_agents = list(node_meta.get("preferred_agents", []))
        read.routing_mode = str(node_meta.get("routing_mode") or "auto")
        read.routing_reason = result_meta.get("routing_reason")
        read.routing_status = result_meta.get("routing_status")
