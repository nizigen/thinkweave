...
async def test_create_task_defaults_to_init_when_no_mid_entry(monkeypatch: pytest.MonkeyPatch):
    async def _fake_plan_task(self, **_kwargs):  # noqa: ARG001
        return (
            SimpleNamespace(
                nodes=[
            ...
            {"pipeline": {"active_stage": "OUTLINE"}},
        )

    monkeypatch.setattr(task_service.PipelineOrchestrator, "plan_task", _fake_plan_task)
...
async def test_create_task_uses_pre_review_integrity_for_draft_entry(
    ...
    assert task_row.checkpoint_data["routing_snapshot"]["strict_bind_failures"] == []
