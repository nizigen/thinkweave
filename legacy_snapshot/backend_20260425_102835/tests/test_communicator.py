...
class TestSendStatusUpdate:
    ...
    @pytest.mark.asyncio
    async def test_status_update_keeps_routing_labels(self, _mock_streams):
        await send_status_update(
            task_id="t1",
            node_id="n1",
            status="running",
        ...
        envelope_arg = _mock_streams["xadd"].call_args[0][1]
        assert envelope_arg.payload["routing_reason"] == "capability_match"
        assert envelope_arg.payload["routing_mode"] == "capability_first"
