...
export default function Home() {
  ...
  const handleSubmit = async (values: CreateTaskPayload) => {
    ...
    try {
      ...
      const routingSnapshot = task.checkpoint_data?.routing_snapshot;
      ...
      if (missingCapabilities.length || strictBindFailures.length) {
        const parts: string[] = [];
        if (missingCapabilities.length) {
          parts.push(`缺失能力 ${missingCapabilities.length} 项`);
        }
        if (strictBindFailures.length) {
          parts.push(`strict bind 未命中 ${strictBindFailures.length} 个节点`);
        }
        message.warning(`路由预检提示：${parts.join('，')}`);
      }
