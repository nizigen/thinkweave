...
describe('Monitor page', () => {
  beforeEach(() => {
    ...
    useMonitorStore.getState().setTaskSnapshot({
      ...
      checkpoint_data: {
        ...
        routing_snapshot: {
          missing_capabilities: ['retrieval'],
          strict_bind_failures: [{ node_id: 'node-2', preferred_agents: ['writer-a'] }],
        },
      },
      nodes: [
        {
  ...
  it('renders the full control tower sections', () => {
    ...
    expect(screen.getByText(/routing: capability_match \(assigned\)/i)).toBeInTheDocument();
    expect(screen.getByText(/routing precheck/i)).toBeInTheDocument();
    expect(screen.getByText(/缺失能力：retrieval/i)).toBeInTheDocument();
    expect(screen.getByText(/strict bind 未命中节点/i)).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'node-2' }));
    expect(useMonitorStore.getState().selectedNodeId).toBe('node-2');
  });
